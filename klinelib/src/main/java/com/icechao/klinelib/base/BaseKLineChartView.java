...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
     ...
     * 执行动画渐变
     */
    private void excuteLastChangeAnim() {
        ...
        for (int i = tempLeft; i <= screenRightIndex; i++) {
            ...
            mainMinValue = status == Status.MA ?
                    mainDraw.getMinValue(mainMinValue,
                            ...
                            points[tempIndex + Constants.INDEX_MA_2],
                            points[tempIndex + Constants.INDEX_MA_3]) :
                    mainDraw.getMaxValue(mainMinValue, points[tempIndex + Constants.INDEX_BOLL_DN]);
...
//
     ...
     */
    public void drawLine(Canvas canvas, Paint paint, float startX, float startValue, float stopX,
                         float stopValue) {
     ...
     * @param stopValue 结束点的值
     */
    public void drawMinutsLineArea(Canvas canvas, Paint paint, float startX, float startValue, float stopX,
                                   float stopValue) {
